...
and writes `sitemap.xml`, `robots.txt` and the blog feed `rss.xml`
(generated from the `## ` sections of `memory/BLOG.md` by `build/make/generate_rss.py` -
each post needs a `*YYYY-MM-DD*` date on its first line). `memory/TOC.md`, the
table of contents, is regenerated by `build/make/generate_toc.py` from the headings
of all pages of the sitemap - do not edit it. The result in
